...
<head>
    ...
    <style>
        ...
        .detail-total-row span:last-child { color: var(--primary-color); }
        ...
        <main class="admin-content">
            <div class="admin-card">
                <div class="page-header">
                    ...
                    <a href="${pageContext.request.contextPath}/admin/order_add.jsp" class="btn btn-secondary" style="display:inline-flex;align-items:center;gap:6px;">
                        &#43; Thêm Đơn Hàng
                    </a>
                </div>
                
                <div style="margin-bottom: 20px;">
                    <form action="${pageContext.request.contextPath}/admin/orders" method="get" style="display: flex; gap: 10px; flex-wrap:wrap;">
                        ...
                        <select name="status" class="form-control" style="width:180px;">
                            ...
                            <option value="PENDING"    ${searchStatus == 'PENDING'    ? 'selected' : ''}>Đang chờ xử lý</option>
                            <option value="PROCESSING" ${searchStatus == 'PROCESSING' ? 'selected' : ''}>Đang xử lý</option>
                            <option value="SHIPPED"    ${searchStatus == 'SHIPPED'    ? 'selected' : ''}>Đang giao</option>
                            <option value="DELIVERED"  ${searchStatus == 'DELIVERED'  ? 'selected' : ''}>Đã giao</option>
                            <option value="CANCELLED"  ${searchStatus == 'CANCELLED'  ? 'selected' : ''}>Đã huỷ</option>
                        </select>
                    ...
                    <tbody>
                        <c:forEach var="o" items="${orders}">
                            <tr>
                                ...
                                <td style="max-width:160px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;" title="${o.diaChiGiaoHang}">${o.diaChiGiaoHang}</td>
                                <td>
                                    <c:choose>
                                ...
                                <td>
                                    <div style="display: flex; gap: 6px; flex-wrap:wrap;">
                                        <!-- Nút xem chi tiết -->
                                        <a href="${pageContext.request.contextPath}/admin/orders?action=detail&id=${o.id}" class="btn-view" title="Xem chi tiết đơn hàng">
                                            <i class="fas fa-eye"></i> Chi tiết
                                        </a>
                                        <!-- Cập nhật trạng thái -->
                                        <form action="${pageContext.request.contextPath}/admin/orders" method="post" style="display:flex; gap:6px;">
                                            <input type="hidden" name="action" value="updateStatus"/>
                                            <input type="hidden" name="id" value="${o.id}"/>
                                            <select name="status" class="form-control" style="width:auto; padding:5px; font-size:13px;">
                                                <option value="PENDING"    ${o.trangThai == 'PENDING'    ? 'selected' : ''}>Chờ xử lý</option>
                                                <option value="PROCESSING" ${o.trangThai == 'PROCESSING' ? 'selected' : ''}>Đang xử lý</option>
                                                <option value="SHIPPED"    ${o.trangThai == 'SHIPPED'    ? 'selected' : ''}>Đang giao</option>
                                                <option value="DELIVERED"  ${o.trangThai == 'DELIVERED'  ? 'selected' : ''}>Đã giao</option>
                                                <option value="CANCELLED"  ${o.trangThai == 'CANCELLED'  ? 'selected' : ''}>Đã huỷ</option>
                                            </select>
                                            <button type="submit" class="btn btn-secondary" style="padding: 5px 10px; font-size:13px;">Cập nhật</button>
                                        </form>
                                        <!-- Xóa -->
                                        <form action="${pageContext.request.contextPath}/admin/orders" method="post">
                                            <input type="hidden" name="action" value="delete"/>
                                            <input type="hidden" name="id" value="${o.id}"/>
                                            <button type="submit" class="btn" style="background: #d9534f; padding: 5px 10px; font-size:13px;" onclick="return confirm('Bạn có chắc muốn xóa đơn hàng này?');">Xóa</button>
                                        </form>
                                    </div>
                                </td>
                            </tr>
